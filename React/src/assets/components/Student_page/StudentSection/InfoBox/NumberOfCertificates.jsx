import React from 'react'
import {FileBadge,  Upload} from 'lucide-react'

const NumberOfCertificates = () => {
  return (
    <div className='h-full w-full gap-6 flex flex-col justify-start'>
        <div className='w-full bg-[#ffffff] outline-1 outline-[#b0b0b0] rounded-lg justify-between relative flex items-center py-4 px-4'>
            <div className='h-[5vh] w-[8%] absolute -top-4 -left-4 overflow-hidden'><img src='./images/61.png' className='h-full w-full object-cover'/></div>

            <div className='flex w-[90%] items-center justify-start'>
                
                <div className='flex w-full flex-col items-start text-[15px] wrap-break-word'>
                    <div className='font-bold text-left leading-tight'>Event name Event name Evnnnnnnnnnnt  </div>
                    <div className='text-[12px] text-[#3f3f3f] mt-1 font-medium flex flex-col gap-3 justify-start items-start'>
                      <div className='font-bold text-[#983530] text-left leading-tight'>by club name club name club name</div>
                      <div className='py-1 px-3 bg-amber-300 rounded-2xl'>Achievement</div>
                      
                    </div>
                </div>
                
            </div>

            <div className='group h-[5vh] w-[10%] ml-4 bg-[#1a3891] outline outline-[#1a3891] hover:bg-[#ffffff] hover:cursor-pointer rounded-lg flex items-center justify-center'>
              <FileBadge size={17} strokeWidth={2} className='group-hover:text-[#1a3891] text-[#ffffff] '/>
            </div>

            
        </div>


        <div className='w-full bg-[#ffffff] outline-1 outline-[#b0b0b0] rounded-lg justify-between relative flex items-center py-4 px-4'>
            {/* <div className='h-[5vh] w-[8%] absolute -top-4 -left-4 overflow-hidden'><img src='./images/61.png' className='h-full w-full object-cover'/></div> */}

            <div className='flex w-[90%] items-center justify-start'>
                
                <div className='flex w-full flex-col items-start text-[15px] wrap-break-word'>
                    <div className='font-bold text-left leading-tight'>Event name Event name Evnnnnnnnnnnt  </div>
                    <div className='text-[12px] text-[#3f3f3f] mt-1 font-medium flex flex-col gap-3 justify-start items-start'>
                      <div className='font-bold text-[#983530] text-left leading-tight'>by club name club name club name</div>
                       <div className='py-1 px-3 bg-amber-300 rounded-2xl'>Participation</div>
                      
                    </div>
                </div>
                
            </div>

            <div className='group h-[5vh] w-[10%] ml-4 bg-[#1a3891] outline outline-[#1a3891] hover:bg-[#ffffff] hover:cursor-pointer rounded-lg flex items-center justify-center'>
              <Upload size={17} strokeWidth={2} className='group-hover:text-[#1a3891] text-[#ffffff] '/>
            </div>

            
        </div>
      </div>
  )
}

export default NumberOfCertificates